...
write.csv(subs,"results/Sept_delayhealth_sublist.csv")
...
inits1 <- dump.format(list( alpha.mu=2.4,   
     ...
     fit <- run.jags(model=model, 
      ...
      time_health_samples <- get_param_matrix("time_health")
      ...
      for (subj in test_subjects) {
        ...
        for (i in seq_len(nrow(subj_trials))) {
          ...
          subframe <- data.frame(cbind(alpha_samples[,subj_idx],b1_samples[,subj_idx],b2_samples[,subj_idx],b3_samples[,subj_idx],theta_samples[,subj_idx],bias_samples[,subj_idx],time_health_samples[,subj_idx]))
          colnames(subframe) <- c("alpha","b1","b2","b3","theta","bias","time_health")
          hd <- subj_trials$hd[i]
          td <- subj_trials$td[i]
          pd <- subj_trials$pd[i]
          ...
          for (iter in 1:n_iter) {
            ...
            tIn_h=subframe$time_health[iter]
            sim_samples[iter] <- ddm3h_parallel(d_v = b1,d_h = b2,d_p = b3,thres = alpha,nDT = theta,bias = bias,vd =td ,hd =hd,pd =pd,tIn_h=tIn_h,N=1,sd_n=1)
          }
      ...
      a$subid <- rownames(a)
      write.csv(a, paste0("results/Sept_delayhealth_cross_validated_fold_", k, ".csv"))
...
write.csv(s,"results/Sept_delayhealth_cross_validated_folds.csv")