...
write.csv(subs,"results/Sept_3FAC_sublist.csv")
...
fold_results <- foreach(k = 1:5, .combine = list, .multicombine = TRUE, .packages = c("runjags", "R2jags", "coda")) %dopar% {
  rm(list=c("train_data","test_data","hd","pd","td","rtpos","ns","idxP","N","y","dat"))
...
dat <- dump.format(list(N=N, y=y, idxP=idxP, hd=hd, td=td, pd=pd, rt=rtpos, ns=ns))
      ...
      inits3 <- dump.format(list( alpha.mu=2,
                                  ...
                                  theta.pr=0.05,  b1.mu=0.3, b1.pr=0.05, b2.mu=0.01, b2.pr=0.05,b3.mu=0.2, b3.pr=0.05, 
                                  bias.mu=0.4,
      ...
      inits2 <- dump.format(list( alpha.mu=2.2, 
                                  ...
                                  theta.pr=0.05, b1.mu=0.3, b1.pr=0.05, b2.mu=0.1, b2.pr=0.05,b3.mu=0.2, b3.pr=0.05,
                                  bias.mu=0.4,
      ...
      inits1 <- dump.format(list( alpha.mu=2.4,   
                                  ...
                                  theta.pr=0.05, b1.mu=0.1, b1.pr=0.05, b2.mu=0.05, b2.pr=0.05,b3.mu=0.2, b3.pr=0.05,
                                  bias.mu=0.4,
     ...
     fit <- run.jags(model=model, 
      ...
      for (subj in test_subjects) {
        ...
        for (i in seq_len(nrow(subj_trials))) {
          ...
          subframe <- data.frame(cbind(alpha_samples[,i],b1_samples[,i],b2_samples[,i],b3_samples[,i],theta_samples[,i],bias_samples[,i]))
          colnames(subframe) <- c("alpha","b1","b2","b3","theta","bias")
          hd <- subj_trials$hd[i]
          td <- subj_trials$td[i]
          pd <- subj_trials$pd[i]
          ...
          for (iter in 1:n_iter) {
            ...
            sim_samples[iter] <- ddm2_parallel(d_v = b1,d_h = b2,d_p=b3,thres = alpha,nDT = theta,bias = bias,vd =td ,hd =hd ,pd=pd,N=1,sd_n=1)
          }
      ...
      a$subid <- rownames(a)
      write.csv(a, paste0("results/Sept_3fac_cross_validated_fold_", k, ".csv"))
...
write.csv(s,"results/Sept_3fac_cross_validated_folds.csv")
