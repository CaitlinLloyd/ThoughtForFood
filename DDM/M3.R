...
# 07/28/25      Claude Code                          Added parallelization optimizations
...
write.csv(subs,"results/Sept_TV_thonly_sublist.csv")
...
fold_results <- foreach(k = 1:5, .combine = list, .multicombine = TRUE, .packages = c("runjags", "R2jags", "coda")) %dopar% {
  ...
  jags_data <- dump.format(list(N=N, y=y, idxP=idxP, hd=hd, td=td, rt=rtpos, ns=ns))
      ...
      time_samples  <- get_param_matrix("time")
      ...
      for (subj in test_subjects) {
        ...
        for (i in seq_len(nrow(subj_trials))) {
          ...
          subframe <- data.frame(cbind(alpha_samples[,i],b1_samples[,i],b2_samples[,i],time_samples[,i],theta_samples[,i],bias_samples[,i]))
          colnames(subframe) <- c("alpha","b1","b2","time","theta","bias")
          hd <- subj_trials$hd[i]
          td <- subj_trials$td[i]
          ...
          for (iter in 1:n_iter) {
            b1=subframe$b1[i]
            b2=subframe$b2[i]
            time=subframe$time[i]
            alpha=subframe$alpha[i]
            theta=subframe$theta[i]
            bias=subframe$bias[i]
            sim_samples[iter] <- ddm2_parallel(d_v = b1,d_h = b2,thres = alpha,nDT = theta,bias = bias,tIn = time,vd =td ,hd =hd ,N=1,sd_n=1)
          }
      ...
      a$subid <- rownames(a)
      write.csv(a, paste0("results/Sept_TV_thonly_cross_validated_fold_", k, ".csv"))
...
write.csv(s,"results/July_TV_thonly_cross_validated_folds.csv")
