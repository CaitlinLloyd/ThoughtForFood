// Time-varying DDM simulation with taste delay relative to health and umami (3-factor model)
...
struct ddm3tw : public Worker {
  ...
  const double tIn_t;      // taste timing relative to health and umami (from Bayesian model)
  GEN gen;
  ...
  ddm3tw(const double d_v, const double d_h, const double d_p, const double thres, const double nDT,
        const double bias, const double vd, const double hd, const double pd, const double sd_n,
        const double tIn_t, NumericVector vecOut, GEN gen)
    : d_v(d_v), d_h(d_h), d_p(d_p), thres(thres), nDT(nDT), bias(bias), vd(vd), hd(hd), pd(pd),
      sd_n(sd_n), tIn_t(tIn_t), gen(gen), vecOut(vecOut) {}
  ...
  void operator()(std::size_t begin, std::size_t end) {
    ...
    // Initialize timing vectors - health and umami are always active from t=0
    ...
    std::vector<double> vec_tTaste(lt, 0);    // taste starts inactive

    // Determine timing based on taste onset parameter
    double abs_tIn_t = std::abs(tIn_t);

    // Set activation times
    int start_t = (int)(abs_tIn_t / dt);  // when taste becomes active

    // Handle different timing scenarios
    if (tIn_t >= 0) {
      // Taste comes after health and umami
      for (int t = start_t; t < lt; t++) {
        vec_tTaste[t] = 1;
      }
    } else {
      // Taste comes before health and umami - health and umami are delayed
      for (int t = 0; t < lt; t++) {
        vec_tTaste[t] = 1;  // taste active from start
      }
      for (int t = 0; t < start_t && t < lt; t++) {
...
// [[Rcpp::export]]
NumericVector ddm3t_parallel(double d_v, double d_h, double d_p, double thres, double nDT, double bias,
                           double vd, double hd, double pd, double sd_n, double tIn_t, unsigned int N) {
  ...
  // create the worker
  ddm3tw ddm3tw(d_v, d_h, d_p, thres, nDT, bias, vd, hd, pd, sd_n, tIn_t, vecOut, gen);

  // call the worker
  parallelFor(0, N, ddm3tw);
