...
write.csv(subs,"results/Sept_3fac_delaytaste_sublist.csv")
...
inits1 <- dump.format(list( alpha.mu=2.4,
     ...
     fit <- run.jags(model=model, 
      ...
      for (subj in test_subjects) {
        ...
        for (i in seq_len(nrow(subj_trials))) {
          ...
          for (iter in 1:n_iter) {
            ...
            sim_samples[iter] <- ddm3t_parallel(d_v = b1,d_h = b2,d_p = b3,thres = alpha,nDT = theta,bias = bias,vd =td ,hd =hd,pd =pd,tIn_t=tIn_t,N=1,sd_n=1)
          }
      ...
      a$subid <- rownames(a)
      write.csv(a, paste0("results/Sept_3fac_delaytaste_cross_validated_fold_", k, ".csv"))
...
write.csv(s,"results/Sept_3fac_delaytaste_cross_validated_folds.csv")
